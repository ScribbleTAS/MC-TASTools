...
//		System.out.println(saver.getMotionSavedX()+" "+saver.getMotionSavedY() +" "+saver.getMotionSavedZ()+" "+ saver.getRelSavedX()+" "+ saver.getRelSavedY()+" "+ saver.getRelSavedZ());
		...
		playeracknowledge.put(player.getName(), new PacketSender(player, saver.getMotionSavedX(), saver.getMotionSavedY(), saver.getMotionSavedZ(), saver.getRelSavedX(), saver.getRelSavedY(), saver.getRelSavedZ(), saver.getPitch(), saver.getYaw()));
	...
	private static void saveFile(MotionSaverServer saver, File file) {
		...
		float pitch=saver.getPitch();
		float yaw=saver.getYaw();
	...
	public MotionSaverServer readFile(File file, String playername) {
		...
		return new MotionSaverServer(playername, x, y, z, rx, ry, rz, fallDistance, pitch, yaw);
	}
	...
	public class PacketSender extends Thread{
		...
		public PacketSender(EntityPlayerMP player,double x, double y, double z, float rx, float ry, float rz, float pitch, float yaw) {
		...
		public void run() {
			while(!breaks) {
				ModLoader.NETWORK.sendTo(new MovementPacket(moX, moY, moZ, relX, relY, relZ, pitch, yaw), playername);
				try {
					Thread.sleep(1000);
