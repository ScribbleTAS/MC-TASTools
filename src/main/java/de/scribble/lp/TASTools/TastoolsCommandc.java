...
public class TastoolsCommandc extends CommandBase{
	...
	public void processCommand(ICommandSender sender, String[] args) throws CommandException  {
		...
		if(sender instanceof EntityPlayer) {
			...
			if (args.length==2&&args[0].equalsIgnoreCase("gui")&&server.getConfigurationManager().getPlayerList().contains(server.getConfigurationManager().getPlayerByUsername(args[1]))) {
	...
	private void guilogoMP(String[] args, MinecraftServer server, ICommandSender sender) {
		...
		ModLoader.NETWORK.sendTo(new MiscPacket(1), server.getConfigurationManager().getPlayerByUsername(args[1]));
	}
	...
	private void savestatetime(String[] args, ICommandSender sender, MinecraftServer server) {
		if (args.length==1) {
			sender.addChatMessage(new ChatComponentTranslation("command.savestatetime.info")); //Set the time it takes to savestate here. Increase the time when playing on big worlds! Usage: /tastools savestatetime <timeinMillis>
			return;
		}else if(args.length==2) {
			try {
				int i= Integer.parseInt(args[1]);
				if(i>50000) {
					sender.addChatMessage(new ChatComponentTranslation("command.savestatetime.toomuch")); //§cThe number is too high! If your world doesn't save correctly with a time below 50000 please contact the author
					return;
				}else if(i<0) {
					sender.addChatMessage(new ChatComponentTranslation("command.savestatetime.toolow")); //Please put in positive numbers!
					return;
				}else if(i<1000) {
					sender.addChatMessage(new ChatComponentTranslation("command.savestatetime.warn")); //Warning! A time lower than 1000 can cause issues with savestating! The suggested value is between 1000 and 5000
				}
				if (!server.isDedicatedServer()) {
					...
					ClientProxy.config.save();
					sender.addChatMessage(new ChatComponentTranslation("command.savestatetime.success",args[1]));
				}else {
				...
				sender.addChatMessage(new ChatComponentTranslation("command.savestatetime.waytoohigh", args[1]));
			}
		}
	}
	...
	private void flintRig(ICommandSender sender) throws CommandException {
		...
		if (sender.canCommandSenderUseCommand(2, "Heck")) {
			if (FlintRig.enable) {
				sender.addChatMessage(new ChatComponentTranslation("msg.flintrig.enable"));	//§aNow only flintdrops
			} else {
				sender.addChatMessage(new ChatComponentTranslation("msg.flintrig.disable"));	//§cBack to normal
			}
		} else {
