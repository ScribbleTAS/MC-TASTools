...
public class TastoolsCommandc extends CommandBase{
	...
	public void execute(MinecraftServer server, ICommandSender sender, String[] args) throws CommandException {
		...
		if(sender instanceof EntityPlayer) {
			...
			if (args.length==2&&args[0].equalsIgnoreCase("gui")&&server.getPlayerList().getPlayerList().contains(server.getPlayerList().getPlayerByUsername(args[1]))) {
			...
			}else if(args.length == 2 && args[0].equalsIgnoreCase("gui")&& server.getPlayerList().getPlayerList()
					.contains(server.getPlayerList().getPlayerByUsername(args[1]))) {
	...
	private void reloadConfig(MinecraftServer server, ICommandSender sender) {
		if (!server.isDedicatedServer() && server.getCurrentPlayerCount() == 1) {
			...
			notifyOperators(sender, this, "msg.misc.reload", new Object());
		} else {
	...
	private void guilogoMP(String[] args, MinecraftServer server, ICommandSender sender) {
		notifyOperators(sender, this, "msg.gui.multiplayerchange", new TextComponentString(args[1]));
		ModLoader.NETWORK.sendTo(new MiscPacket(1), server.getPlayerList().getPlayerByUsername(args[1]));
	}
	...
	private void gameover(ICommandSender sender) {
		...
		sender.addChatMessage(new TextComponentTranslation(msg));
	}
	private void savestatetime(String[] args, ICommandSender sender, MinecraftServer server) {
		if (args.length==1) {
			sender.addChatMessage(new TextComponentTranslation("command.savestatetime.info")); //Set the time it takes to savestate here. Increase the time when playing on big worlds! Usage: /tastools savestatetime <timeinMillis>
			return;
		}else if(args.length==2) {
			try {
				int i= Integer.parseInt(args[1]);
				if(i>50000) {
					sender.addChatMessage(new TextComponentTranslation("command.savestatetime.toomuch")); //§cThe number is too high! If your world doesn't save correctly with a time below 50000 please contact the author
					return;
				}else if(i<0) {
					sender.addChatMessage(new TextComponentTranslation("command.savestatetime.toolow")); //Please put in positive numbers!
					return;
				}else if(i<1000) {
					sender.addChatMessage(new TextComponentTranslation("command.savestatetime.warn")); //Warning! A time lower than 1000 can cause issues with savestating! The suggested value is between 1000 and 5000
				}
				if (!server.isDedicatedServer()) {
					...
					ClientProxy.config.save();
					sender.addChatMessage(new TextComponentTranslation("command.savestatetime.success",args[1]));
				}else {
				...
				sender.addChatMessage(new TextComponentTranslation("command.savestatetime.waytoohigh", args[1]));
			}
		}
	}
	...
	private void flintRig(ICommandSender sender) throws CommandException {
		...
		if (sender.canCommandSenderUseCommand(2, "Heck")) {
			if (FlintRig.enable) {
				sender.addChatMessage(new TextComponentTranslation("msg.flintrig.enable"));	//§aNow only flintdrops
			} else {
				sender.addChatMessage(new TextComponentTranslation("msg.flintrig.disable"));	//§cBack to normal
			}
		} else {
